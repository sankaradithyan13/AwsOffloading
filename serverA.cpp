...
using namespace std;
#define SERVERAPORT 21227 //server A static port number declaration
#define AWSPORT 24227 //AWS server static port number declaration
int main( void )
{
    //variable initializations
    int fd; //variable to store socket descriptor
    ifstream file("database_a.csv"); //take input database file
	ifstream file1("database_a.csv"); //take a copy of the database file
    ...
    { 
        perror( "Server A socket creation failed" ); //return error if there is error in socket creation
        return 1;
    }

    //server A details
    struct sockaddr_in serverAaddr; //initialize structure for server A
    memset(&serverAaddr, 0, sizeof(serverAaddr)); //clear the memory contents of structure
    serverAaddr.sin_family = AF_INET; //address family is internet
    serverAaddr.sin_port = htons(SERVERAPORT); //port number of server A is static and defined above
    serverAaddr.sin_addr.s_addr = htonl( 0x7f000001 ); //IP address of server A
    printf("The server A is up and running using UDP on port <%d>\n",ntohs(serverAaddr.sin_port)); //print server is running on successful creation

    //binding server A address
    if ( bind(fd, (struct sockaddr *)&serverAaddr, sizeof(serverAaddr)) < 0 ) //bind the address of the server A to the socket descriptor
    { 
        perror( "Server A bind failed" ); //return error if there is error in binding socket
        return 1;
    }
    ...
    //store the extracted parameters from file to other variables
    for(int k=1;k<a;k++)
    {
    ...
    while(1) //while loop to keep server continuously running to listen for inputs from client
    {
		...
		if ( recvfrom(fd,buffer,sizeof(buffer),0, (struct sockaddr *)&awsaddr, (socklen_t *)&slenAWS) < 0 ) //receive data from aws server
		{
	    ...
	    for(int j=0;j<i;j++)
	    {
	    	...
	    	{	
	    		...
	    		if(sendto(fd,"ServerA",10, 0, (struct sockaddr *)&awsaddr, sizeof(awsaddr)) > 0) //send keyword to aws server if data is found in database
	    		{
	    			cout<<"The server A has found <1> match"<<endl; //display when match is found
		...
		{
            if(sendto(fd,"notServerA",10, 0, (struct sockaddr *)&awsaddr, sizeof(awsaddr))>0) //send keyword to aws server if data is not found in database
            {
                cout<<"The server A has found <0> match"<<endl; //display when no match is found
                printf( "The Server A finished sending the output to AWS\n"); //display message that server A has finished sending output
            }
            else
            {
